...
export const Certifications = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("/job-seeker/create-profile/certification", formData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem("token")}`
                }
            }).then(
                response => {
                    SweetAlert('success', 'Update Successful', 'Your Certification information has been updated successfully');
